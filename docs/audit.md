...
| A04 | High | Generic sink deinit is void; I2S retains state when DROP fails, but graph release cannot learn that. Native LMS cleanup previously ignored stop/join/deinit errors (now retained/reported); the generic sink deinit gap remains. | Open, T03. Define recoverable/fatal cleanup and retain ownership on errors. Add injected driver-failure tests before changing API. |
