...
| A03 | High | `platform/native_linux/play_lms.c:start` creates the audio thread after pipeline START; the Linux HAL allocates for it. | Fixed under T02: create/gate worker before START, then release it. Direct heap-call guard and lifecycle failure tests added. |
| A04 | High | Generic sink deinit is void; I2S retains state when DROP fails, but graph release cannot learn that. Native LMS cleanup previously ignored stop/join/deinit errors. | Fixed in T03: checked deinit, partial-init retention, START rollback and recovery state; injected host and Zephyr driver errors exercise retry. |
...
| A07 | Medium | Native player feature `#ifdef EAF_HAVE_ALSA` branches and POSIX CLI includes live in platform code. | Fixed in T01: CMake-selected native output adapters; v0.5 explicitly scopes the POSIX CLI exception. Source guards cover public headers and native player/adapter C. |
...
| A09 | Medium | Linux semaphore deadlines use CLOCK_REALTIME; wall-clock changes can perturb waits. Zephyr thread priority is one global setting with no affinity API. | Software fixed in T04: monotonic semaphore deadlines, distinct role priorities, optional affinity and failure/contended-wake tests. ESP32 scheduling margin remains unmeasured. |
| A10 | Medium | ALSA write/drain waits are bounded but far longer than an audio block; plugins may allocate and buffered delay is not speaker-time proof. I2S EOS now requests DRAIN and waits up to one second for all four slab blocks; explicit STOP still drops. | Driver-queue lifecycle implemented in T03; T09/T21 still require physical EOF-tail and timing measurements. |
